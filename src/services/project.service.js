...
class ProjectService {
    ...
    async getProjectService() {
        ...
        let projectsData = await ProjectModel.find(match).lean();
        if (!projectsData.length) throw Error("Project Not Found!");
        ...
        let groupsData = [];
        if (groupIDs.size)
            groupsData = await GroupModel.aggregate([
                {
                    $match: {
                        _id: { $in: [...groupIDs] }
                    }
        ...
        return projectsData.map(element => ({
            ...element,
            projectGroups: element.projectGroups.map(item => groupsData.find(x => item.equals(x._id)) || item)
        }));
    }
